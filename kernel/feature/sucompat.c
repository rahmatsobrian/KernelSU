...
// - the susfs-patched kernel reads this key directly in its inline hooks
...
#else
#ifdef CONFIG_KSU_SUSFS
	if (!static_key_enabled(&ksu_su_compat_enabled.key))
		return false;
#else
...
{
#ifdef CONFIG_KSU_SUSFS
	*value = static_key_enabled(&ksu_su_compat_enabled.key) ? 1 : 0;
#else
...
#ifdef CONFIG_KSU_SUSFS
	if (enable == static_key_enabled(&ksu_su_compat_enabled.key)) {
#else
...
#ifdef CONFIG_KSU_SUSFS
// === susfs sucompat entry points, called inline by the susfs-patched kernel ===
static const char susfs_sh_path[] = SH_PATH;
static const char susfs_su_path[] = SU_PATH;
static const char susfs_ksud_path[] = KSUD_PATH;

static __always_inline bool susfs_name_is_su(const char *name)
{
	uintptr_t *su_p = (uintptr_t *)susfs_su_path;
	uintptr_t *fn_p = (uintptr_t *)name;

	// getname() hands out full page slots, so a 16-byte overread is fine
#ifdef CONFIG_64BIT
	if (likely((fn_p[1] & 0x00FFFFFFFFFFFFFFUL) != (su_p[1] & 0x00FFFFFFFFFFFFFFUL)))
		return false;
#else
	if (likely(fn_p[3] != su_p[3]))
		return false;
	if (unlikely(get_unaligned((uint32_t *)(name + 12)) != get_unaligned((uint32_t *)(susfs_su_path + 12))))
		return false;
#endif
	if (unlikely(fn_p[0] != su_p[0]))
		return false;
...
// returns 0 when this exec was a su session (kernel then calls the post hook)
int ksu_handle_execveat_sucompat(int *fd, struct filename **filename_ptr, void *argv, void *envp, int *flags)
{
...
#ifdef CONFIG_KSU_FEATURE_ADBROOT
	ksu_adb_root_execve_kernel((void *)&filename->name, envp);
#endif

	if (!is_su_allowed((const void **)&filename->name))
		return -EINVAL;
	...
	if (!susfs_name_is_su(filename->name))
		return -EINVAL;

	if (current_chrooted()) {
		pr_err("su_compat: su found but NOT allowed! Because current process is running in chrooted environment\n");
		return -EINVAL;
	}
	...
	// NOTE: we only check file existence, not exec success!
	// the su session fd is installed post-exec by ksu_handle_post_execveat_sucompat()
	struct path kpath;
	if (!!kern_path(KSUD_PATH, 0, &kpath)) {
		pr_info("su_compat: do_execveat_common su->sh!%s\n", (is_compat_task()) ? " [compat]" : "");
		memcpy((void *)filename->name, susfs_sh_path, sizeof(susfs_sh_path));
	} else {
		path_put(&kpath);
		pr_info("su_compat: do_execveat_common su->ksud!%s\n", (is_compat_task()) ? " [compat]" : "");
		memcpy((void *)filename->name, susfs_ksud_path, sizeof(susfs_ksud_path));
	}
...
int ksu_handle_faccessat(int *dfd, struct filename **filename_ptr, int *mode, int *__unused_flags)
{
	...
	if (likely(!susfs_name_is_su(filename->name)))
		return 0;
