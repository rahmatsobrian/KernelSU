...
// - moved to file scope so the susfs inline input hook can share them
...
#ifdef CONFIG_KSU_SUSFS
	if (static_key_enabled(&ksu_is_init_rc_hook_enabled.key)) {
...
#ifdef CONFIG_KSU_SUSFS
	if (static_key_enabled(&ksu_is_input_hook_enabled.key)) {
...
{
#ifdef CONFIG_KSU_SUSFS
	// input events reach us via ksu_handle_input_handle_event() from the
	// susfs-patched kernel instead of a registered input handler
#else
	vol_detector_init();
#endif
}
...
__attribute__((cold)) int ksu_handle_input_handle_event(unsigned int *type, unsigned int *code, int *value)
{
	...
	// - we run within the input device spinlock here, so we must NOT call
	//   static_branch_disable() from this context. the key gets disabled by
	//   stop_input_hook() from ksud / on_post_fs_data() instead.
	if (vol_up_cnt >= VOLUME_PRESS_THRESHOLD_COUNT || vol_down_cnt >= VOLUME_PRESS_THRESHOLD_COUNT) {
		pr_info("volume keys pressed max times, safe mode detected!\n");
		safe_mode_flag = true;
	}
...
// called from ksys_read() in the susfs-patched kernel
__attribute__((cold)) void ksu_handle_sys_read(unsigned int fd)
{
	ksu_handle_sys_read_fd(fd);
}

// called from vfs_fstat() in the susfs-patched kernel, bumps init.rc size in kstat
void ksu_handle_vfs_fstat(int fd, loff_t *kstat_size_ptr)
{
	...
	*kstat_size_ptr += (loff_t)(ksu_rc_len + module_rc_len);
	pr_info("added rc len: +%zu (static=%zu module=%zu)\n", ksu_rc_len + module_rc_len, ksu_rc_len, module_rc_len);
}
